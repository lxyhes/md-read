...
<template>
  <div v-if="props.compact" class="theme-picker-compact" aria-label="阅读主题">
    <button v-for="theme in props.themes" :key="theme.manifest.id" class="theme-mini-option" :class="{ selected: props.selectedThemeId === theme.manifest.id }" :style="themeVars(theme)" type="button" :aria-pressed="props.selectedThemeId === theme.manifest.id" :title="`切换到${theme.manifest.name}`" @click="selectTheme(theme)">
  ...
  <div v-else class="theme-picker-gallery">
    <button v-for="theme in props.themes" :key="theme.manifest.id" class="theme-card" :class="{ selected: props.selectedThemeId === theme.manifest.id }" :style="themeVars(theme)" type="button" @click="selectTheme(theme)">
